...
haven't looked at it yet. What follows is older ranting from before I
knew of that link.

  There doesn't appear to be a Google Photos API.  There is, however,
  a picasaweb API: https://developers.google.com/picasa-web/.  That
  page says "Beginning May 1st, 2016, we’ll start rolling out changes
  to the Picasa Web Albums Data API and no longer support the
  following functionality" Last I checked, it was December 2016.
  Everything about this site seems abandoned.
  ...
  My remaining hope is that I can upload photos to some sort of
  generic data-uploading API such as
  https://cloud.google.com/storage/, which as best I can tell from the
  marketing gibberish, is roughly equivalent to Amazon's S3.  There
  appears to be a python client at
...
# years; get a horse"
print ('<a href="%s">Login to your Google account</a>'.format(authSubUrl))
